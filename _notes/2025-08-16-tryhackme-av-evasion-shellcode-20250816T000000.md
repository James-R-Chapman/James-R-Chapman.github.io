---
layout: post
title: "TryHackMe  - AV Evasion - Shellcode"
date: 2025-08-16
tags: [["tryhackme"]]
categories: [tryhackme]
hubs: "TryHackMe/Red Teaming/Host Evasions"
identifier: "20250816T000000"
source_id: "2b4456f9-53b0-4781-8b97-cddff5e67748"
source_urls: "- (https://tryhackme.com/room/avevasionshellcode)"
...
Start MachineIn this challenge, we prepared a Windows machine with a web application to let you upload your payloads. Once uploaded, the payloads will be checked by an AV and executed if found to be clean of malware. The main goal of this challenge is to evade Antivirus software installed on the VM and capture the flag in the file system. Feel free to try all of the techniques discussed throughout the room by uploading them to `http://MACHINE_IP/`.
...
- Once the AV scans the uploaded file and no malicious code is detected, the file gets executed. Thus, if everything is put together correctly, then you should receive a reverse shell.
 ...
 ![Image 1](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/7a5c9d8d5501eaed9d0b677d94266414.png)
...
![Image 2](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/94fe3c0f556877a2721ca9e0744ad026.png)

   **Username** thm **Password** Password321     You will also need the AttackBox for some tasks, so this is also a good moment to start it.
...
![Image 3](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/ad61ff4aa1d4f649c02348dfa32eb613.png)
...
![Image 5](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/c51856efd63b36680857498bac814469.png)
...
![Image 6](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/78dca06d1d1e4249f25734af8082b8be.png)
...
**Question:** What is the last 6 digits of the MD5 hash value of the thm-intro2PE.exe file?

*Answer:* 

     530949

**Question:** What is the Magic number value of the thm-intro2PE.exe file (in Hex)?

*Answer:* 

     5A4D

**Question:** What is the Entry Point value of the thm-intro2PE.exe file?

*Answer:* 

     12E4

**Question:** How many Sections does the thm-intro2PE.exe file have?
...
![Image 7](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/7d8572294a70c545705aecc6c2453a3e.png)
...
**rax** **System Call** **rdi** **rsi** **rdx** 0x1sys_writeunsigned int fdconst char *bufsize_t count0x3csys_exitint error_code
...
**Question:** Modify your C program to execute the following shellcode. What is the flag?
...
While your machine's AV should be disabled, feel free to try and upload your payload to the THM Antivirus Check at `http://MACHINE_IP/`.
 ...
 ![Image 8](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/e90959ae18f71d51d7b8681676029493.png)
...
**Question:** Apply what we discussed in this task and get ready for the next topic!
...
![Image 9](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/cad28e045fd6fec615b04d731aef7f9a.png)
...
![Image 10](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/f92f294a9e599967a0961b4273381be6.png)
...
![Image 11](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/fd8a98b3cb79cca98a1e0dfd0292ea61.png)
...
Stageless payloadwindows/x64/shell/reverse_tcp
Staged payloadYou will generally find that the same name patterns are applied to other types of shells. To use a stageless Meterpreter, for example, we would use the `windows/x64/meterpreter_reverse_tcp`, rather than `windows/x64/meterpreter/reverse_tcp`, which works as its staged counterpart.
...
**WinAPI Function** **Description** [VirtualAlloc()](https://docs.microsoft.com/en-us/windows/win32/api/memoryapi/nf-memoryapi-virtualalloc)Allows us to reserve some memory to be used by our shellcode.[CreateThread()](https://docs.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread)Creates a thread as part of the current process.[WaitForSingleObject()](https://docs.microsoft.com/en-us/windows/win32/api/synchapi/nf-synchapi-waitforsingleobject)Used for thread synchronization. It allows us to wait for a thread to finish before continuing.The part of the code in charge of importing these functions is the following:
...
**Question:** Do staged payloads deliver the full content of our payload in a single package? (yea/nay)

*Answer:* 

     nay

**Question:** Is the Metasploit payload windows/x64/meterpreter_reverse_https a staged payload? (yea/nay)

*Answer:* 

     nay

**Question:** Is the stage0 of a staged payload in charge of downloading the final payload to be executed? (yea/nay)

*Answer:* 

     yea

**Question:** Follow the instructions to create a staged payload and upload it into the THM Antivirus Check at http://MACHINE_IP/
...
![Image 12](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/6b3d06034f60f472a3c3620815d25be1.png)
...
![Image 13](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/7cd2d23f048dd314a46910fb29c9836f.png)
...
**Question:** Is encoding shellcode only enough to evade Antivirus software? (yea/nay)

*Answer:* 

     nay

**Question:** Do encoding techniques use a key to encode strings or files? (yea/nay)

*Answer:* 

     nay

**Question:** Do encryption algorithms use a key to encrypt strings or files? (yea/nay)
...
![Image 14](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/747c69e737c96044123329c47845f659.png)
...
**Question:** Try to use this technique (combining encoding and encryption) on the THM Antivirus Check at http://MACHINE_IP/. Does it bypass the installed AV software?
...
- Protect the program from reverse engineering in general.
 ...
 ![Image 15](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/9aacb2fab2a656ffc82a7b0344918062.png)
 ...
 ![Image 16](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/408fb909374c2b54bebef9809eaa417a.png)
...
```
 ...
 ![Image 17](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/9214df2a88ffffbf8561502aa19a375c.png)
 ...
 ![Image 18](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/857e5540e14f4ebf743fbd4d2f8ee503.png)

 We will now edit the "true" rule and set it to the Maximum preset:

 ![Image 19](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/96946f1aff585a91e78408b96e446ea4.png)

 Finally, we will go to the "Protect!" tab and hit "Protect":

![Image 20](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/e0e3ca97245641d3954f26fa986aae87.png)
...
**Question:** Will packers help you obfuscate your malicious code to bypass AV solutions? (yea/nay)

*Answer:* 

     yea

**Question:** Will packers often unpack the original code in-memory before running it? (yea/nay)

*Answer:* 

     yea

**Question:** Are some packers detected as malicious by some AV solutions? (yea/nay)

*Answer:* 

     yea

**Question:** Follow the instructions to create a packed payload and upload it into the THM Antivirus Check at http://MACHINE_IP/
...
While not an AV bypass method, binders are also important when designing a malicious payload to be distributed to end users. A **binder**  is a program that merges two (or more) executables into a single one. It is often used when you want to distribute your payload hidden inside another known program to fool users into believing they are executing a different program.
 ...
 ![Image 21](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/5c72d80a077a1f8a813a70c01a6561e9.png)
...
**Question:** Will a binder help with bypassing AV solutions? (yea/nay)

*Answer:* 

     nay

**Question:** Can a binder be used to make a payload appear as a legitimate executable? (yea/nay)
