---
layout: post
title: "TryHackMe  - Data Exfiltration"
date: 2025-08-12
tags: [["tryhackme"]]
categories: [tryhackme]
hubs: "TryHackMe/Red Teaming/Post Compromise"
identifier: "20250812T000000"
source_id: "6faa48f3-6948-4a18-8a2f-823eb191e0dd"
source_urls: "- (https://tryhackme.com/room/dataxexfilt)"
...
Start MachineNetwork Infrastructure
...
![Image 1](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/224e0380ac936c602fe41c6537ed4565.png)
...
**Domain Name** **IP Address** **Network Access** jump.thm.com192.168.0.133Net 1 and Net 2uploader.thm.com172.20.0.100
Net 1flag.thm.com
*****.**.*.***** Net 1victim2.thm.com
172.20.0.101
Net 1web.thm.com
192.168.0.100
Net 2icmp.thm.com
192.168.0.121
Net 2victim1.thm.com
192.168.0.101
Net 2 
...
**Question:** Once you've deployed the VM, please wait a few minutes for the VM and the networks to start, then progress to the next task!
...
Data Exfiltration is the process of taking an unauthorized copy of sensitive data and moving it from the inside of an organization's network to the outside. It is important to note that Data Exfiltration is a post-compromised process where a threat actor has already gained access to a network and performed various activities to get hands on sensitive data. Data Exfiltration often happens at the last stage of the Cyber Kill Chain model, Actions on Objectives.
 ...
 ![Image 2](https://tryhackme-images.s3.amazonaws.com/user-uploads/5ed5961c6276df568891c3ea/room-content/5ed5961c6276df568891c3ea-1721309536895.png)
...
![Image 3](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/0c3438995ccff35a5589b9abd3703b14.png)
...
![Image 4](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/49ad248f2506a5a749dbb70732c32072.png)
...
![Image 5](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/b4c99b2aba13eac24379fee2d20ffbf6.png)
...
**Question:** In which case scenario will sending and receiving traffic continue during the connection?

*Answer:* 

     Tunneling

**Question:** In which case scenario will sending and receiving traffic be in one direction?

*Answer:* 

     traditional data exfiltration

**Question:** In the next task, we will be discussing how data exfiltration over the TCP socket works!
...
![Image 6](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/9931b598f5757bbdfb74004a2a43fe16.png)
...
**Question:** Exfiltration using TCP sockets relies on ____________ protocols!

*Answer:* 

     non-standard

**Question:** Now apply what we discussed to exfiltrate data over the TCP socket! Once you exfiltrate data successfully, hit Completed to move on to the next task!
...
![Image 7](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/aa723bb0e2c39dfc936b135c4912d1cf.png)
...
**Question:** All packets sent using the Data Exfiltration technique over SSH are encrypted! (T=True/F=False)

*Answer:* 

     T

**Question:** Replicate the steps to transfer data over the SSH client. Once you transfer the file successfully, hit Completed and move on to the next task!
...
![Image 8](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/789a9a13d9977b11d11f53bb7dbb9f3a.png)
...
![Image 9](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/fbf6c90063102ca100ba8d544ba9d7f8.png)
...
![Image 10](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/92004a7c6a572f9680f0056b9aa88baa.png)
...
![Image 11](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/2445ab5b8bc971b51f8ebe3ffbd0c07d.png)
...
```
 ...
 ![Image 12](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/b32d6d9d9c3377acf155044204ec6982.png)
...
**Question:** Check the Apache log file on web.thm.com  and get the flag!

*Answer:* 

     THM{H77P-G37-15-f0un6}

**Question:** When you visit the http://flag.thm.com/flag website through the uploader machine via the HTTP tunneling technique, what is the flag?
...
Network devices such as routers use ICMP protocol to check network connectivities between devices. Note that the ICMP protocol is not a transport protocol to send data between devices. Let's say that two hosts need to test the connectivity in the network; then, we can use the ping command to send ICMP packets through the network, as shown in the following figure.
 ...
 ![Image 13](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/2a65a034de59c6e603a5a5f61fd7d909.png)
...
On a high level, the ICMP packet's structure contains a Data section that can include strings or copies of other information, such as the IPv4 header, used for error messages. The following diagram shows the Data section, which is optional to use.
 ...
 ![Image 14](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/38e7df5e059ece4c2567bd7f77421b22.png)
...
```
 ...
 ![Image 15](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/3e8367a535e3f7f4076986987b9e0dcd.png)
...
![Image 16](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/61d50ec683ddae6c2f52f532cc02f685.png)
...
```
 ...
 ![Image 17](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/6a086470f770c67c0a07f9572088e5e1.png)
...
![Image 18](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/b45715c44b5998fa9bf6a989b1e0d8d6.png)
...
![Image 19](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/c4c0b7beeaa41fd5b4a4f4cbe1ded82e.png)
...
![Image 20](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/b7df6f586e47769bf2addbee68d69cdc.png)

### **Answer the questions below**

**Question:** In which ICMP packet section can we include our data?

*Answer:* 

     Data

**Question:** Follow the technique discussed in this task to establish a C2 ICMP connection between JumpBox and ICMP-Host. Then execute the "getFlag" command. What is the flag?
...
![Image 21](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/b8b4c25e0eb4dd04f1aea8596bf9319e.png)

Once you choose the domain name, you can add DNS records and test and reset the DNS configuration if something goes wrong.

![Image 22](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/af468d0202712ac1890f5dacb135e532.png)
...
**Domain Name** **IP Address** **Network Access** attacker.thm.com
172.20.0.200Network 2We will be using the Attacker machine to exfiltrate in DNS and DNS tunneling scenarios. The main goal is that the Attacker machine (on Network2) can access internal network devices of Network 1 through JumpBox.

![Image 23](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/e6bf2c81281be5cf8515eeed22254643.png)
...
**DNS Record** **Type** **Value** attNS.tunnel.com
A172.20.0.200
att.tunnel.comNSattNS.tunnel.com
...
**Question:** Once the DNS configuration works fine, resolve the flag.thm.com  domain name. What is the IP address?
...
![Image 24](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/8bbc858294e45de16712024af22181fc.png)
...
![Image 25](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/9881e420044ca01239d34c858342b888.png)
...
![Image 26](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/e881336d12bd5f24d2167730adda0adc.png)
...
![Image 27](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/a7ac15da0501d577dadcf53b4143ff98.png)
...
![Image 28](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/38b87cfbbe254bef1e98f0dffa49451f.png)
...
**Question:** What is the maximum length for the subdomain name (label)?

*Answer:* 

     63

**Question:** The Fully Qualified FQDN domain name must not exceed ______ characters.

*Answer:* 

     255

**Question:** Execute the C2 communication over the DNS protocol of the flag.tunnel.com. What is the flag?
...
DNS Tunneling (TCPoverDNS)
 ...
 ![Image 29](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/8176731af9ec61cf248cdbc65df92172.png)
...
![Image 30](https://tryhackme-images.s3.amazonaws.com/user-uploads/5d617515c8cd8348d0b4e68f/room-content/ffbd2ecb2563c649fde174b40c450097.png)
...
**Question:** When the iodine connection establishes to Attacker, run the ifconfig command. How many interfaces are? (including the loopback interface)

*Answer:* 

     4

**Question:** What is the network interface name created by iodined?

*Answer:* 

     dns0

**Question:** Use the DNS tunneling to prove your access to the webserver, http://192.168.0.100/test.php . What is the flag?
...
**Question:** Read the closing task.
