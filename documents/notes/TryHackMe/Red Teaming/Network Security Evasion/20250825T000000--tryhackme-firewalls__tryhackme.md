...
URLs: (https://tryhackme.com/room/redteamfirewalls)
id: 8b0d49a1-2009-43ef-a434-3051eca3e2f0
---
...
- Destination Address
 ...
 ![Image 1](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/09d8061f603e6ba8e65a185dc4a2d417.png)
...
- Destination Port Number
 ...
 ![Image 2](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/756fac51ff45cbc4af49336b15a30928.png)

 Learning Objectives This room covers:
...
- Reverse and bind shells. We recommend the [What the Shell?](https://tryhackme.com/room/introtoshells) room to learn more about shells.
 ...
 Warmup Questions The design logic of traditional firewalls is that a port number would identify the service and the protocol. In traditional firewalls, i.e., packet-filtering firewalls, everything is allowed and blocked mainly based on the following:
...
- Destination TCP or UDP port number
 ...
 Visit [Service
Name and Transport Protocol Port Number Registry](http://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xhtml) to learn more about the default port number and to answer the following questions.
...
![Image 3](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/b496d0fb1bec2be05d7202e2ddbf1663.png)
...
![Image 4](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/ad200ff1a857631d88940d3e3637736b.png)
...
5. Fixed Source Port Number
 ...
 ![Image 5](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/169fd944d79366e156fcb6c30ff8018e.png)
...
**Question:** Fixed Source Port Number
...
**Question:** This is a quick summary of the Nmap options discussed in this task.
...
**Question:** Fragment Your Packets with 8 Bytes of Data
...
**Question:** Fragment Your Packets with 16 Bytes of Data
...
**Question:** Fragment Your Packets According to a Set MTU
...
**Question:** Generate Packets with Specific Length
...
Start MachineNmap allows you to control various header fields that might help evade the firewall. You can:
...
- Send packets with a wrong TCP/UDP checksum
 ...
 Set TTL Nmap gives you further control over the different fields in the IP header. One of the fields you can control is the Time-to-Live (TTL). Nmap options include `--ttl VALUE` to set the TTL to a custom value. This option might be useful if you think the default TTL exposes your port scan activities.
 ...
 ![Image 6](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/f98efaf6faf449bf6cc2787baa581e31.png)
...
Start MachineThree common firewall evasion techniques are:
...
- Use of non-standard ports
 ...
 ![Image 7](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/26fce8aa8569f391ad64a26a147de2d4.png)
...
Port tunneling is also known as *port forwarding*  and *port mapping* . In simple terms, this technique forwards the packets sent to one destination port to another destination port. For instance, packets sent to port 80 on one system are forwarded to port 8080 on another system.
 ...
 Port Tunneling Using `ncat` Consider the case where you have a server behind the firewall that you cannot access from the outside. However, you discovered that the firewall does not block specific port(s). You can use this knowledge to your advantage by tunneling the traffic via a different port.
...
- `"ncat TARGET_SERVER 25"` will connect to the target server at port 25.
 ...
 ![Image 8](https://tryhackme-images.s3.amazonaws.com/user-uploads/5f04259cf9bf5b57aed2c476/room-content/ef6b903dbb6c4eb20051f9ddd5b9fa8f.png)
...
This room covered the different types of firewalls and the common evasion techniques. Correctly understanding the limitations of the firewall technology you are targeting helps you pick and construct suitable firewall evasion processes. This room demonstrated different evasion techniques using `ncat`; however, the same results can be achieved using a different tool, such as `socat`. It is recommended to check out the [What the Shell?](https://tryhackme.com/room/introtoshells) room.
 ...
 The following table summarizes the Nmap arguments covered in this room.
    ...
    Evasion Approach Nmap Argument     Hide a scan with decoys `-D DECOY1_IP1,DECOY_IP2,ME`   Use an HTTP/SOCKS4 proxy to relay connections `--proxies PROXY_URL`   Spoof source MAC address `--spoof-mac MAC_ADDRESS`   Spoof source IP address `-S IP_ADDRESS`   Use a specific source port number `-g PORT_NUM` or `--source-port PORT_NUM`   Fragment IP data into 8 bytes `-f`   Fragment IP data into 16 bytes `-ff`   Fragment packets with given MTU `--mtu VALUE`   Specify packet length `--data-length NUM`   Set IP time-to-live field `--ttl VALUE`   Send packets with specified IP options `--ip-options OPTIONS`   Send packets with a wrong TCP/UDP checksum `--badsum`

### **Answer the questions below**

**Question:** Ensure you have gained a solid understanding of the technologies and techniques presented in this room.
